...
from history_api.presidents_id import get_president_by_id, LANGUAGES
...
def test_should_work_with_capitalized_lang():
    lang = 'EN'
    id = '5'
    result = get_president_by_id(lang, id)
    assert result
...
def test_get_president_id_less_than_1():
    ...
    invalid_id = '0'
    with raises(KeyError) as error:
        get_president_by_id(lang, invalid_id)
    assert 'Provided president ID does not exist' in str(error.value)
